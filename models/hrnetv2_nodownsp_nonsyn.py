...
__all__ = ['hrnetv2_nodownsp_nonsyn']
        ...
        if stride != 1 or \
           self.num_inchannels[branch_index] != num_channels[branch_index] * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(num_channels[branch_index] * block.expansion, track_running_stats=False),
            )
                ...
                if j > i:
                    fuse_layer.append(nn.Sequential(
                        nn.Conv2d(num_inchannels[j],
                                  ...
                                  0,
                                  bias=False),
                        nn.BatchNorm2d(num_inchannels[i], track_running_stats=False)))
                elif j == i:
                        ...
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(nn.Sequential(
                                nn.Conv2d(num_inchannels[j],
                                          ...
                                          3, 2, 1, bias=False),
                                nn.BatchNorm2d(num_outchannels_conv3x3, track_running_stats=False)))
                        else:
...
class HRNetV2(nn.Module):
    def __init__(self, n_class, **kwargs):
        ...
        self.cb = 64
        # stem net
        self.conv1 = nn.Conv2d(3, self.cb, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(self.cb, track_running_stats=False)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn2 = nn.BatchNorm2d(64, track_running_stats=False)
        self.relu = nn.ReLU(inplace=True)

        self.layer1 = self._make_layer(Bottleneck, self.cb, self.cb, 4)
        ...
        self.transition1 = self._make_transition_layer([self.cb*4], num_channels)
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.Sequential(
                        nn.Conv2d(num_channels_pre_layer[i],
                                  ...
                                  bias=False),
                        nn.BatchNorm2d(
                            num_channels_cur_layer[i], track_running_stats=False),
                        nn.ReLU(inplace=True)))
                else:
...
def hrnetv2_nodownsp_nonsyn(pretrained=False, **kwargs):
