...
cdef extern from "stdio.h":
    ...
    extern void *stderr
    extern void *stdout
    extern stdio.FILE *open_memstream(char **, size_t *)
...
cdef class _Qhull:
    ...
    def __init__(self,
        ...
        self.handle = open_memstream(&self._memstream_ptr,
        ...
        with nogil:
            ...
            exitcode = qh_new_qhull_scipy(self._qh, self.ndim, self.numpoints,
                                          <realT*>points.data, 0,
                                          options_c, NULL, self.handle, coord)
    ...
    def add_points(self, points, interior_point=None):
        ...
        try:
            ...
            if exitcode != 0:
                raise QhullError(self.get_message())
...
class Delaunay(_QhullUser):
    ...
    @cython.boundscheck(False)
    def find_simplex(self, xi, bruteforce=False, tol=None, return_c=False):
        """
        ...
        c_all = np.zeros((xi.shape[0], self.ndim+1), dtype=np.double)
        c_all_ = c_all
        ...
        if return_c==False:
            ...
            return out.reshape(xi_shape[:-1]), c_all
